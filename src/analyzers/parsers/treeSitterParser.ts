...
export class TreeSitterParser {
  ...
  private initializeParsers(): void {
    try {
      ...
      tsParser.setLanguage(TypeScript as unknown as Parser.Language);
  ...
  private async buildCodeGraph(
    ...
    switch (language) {
      case "typescript":
      case "javascript":
        await this.extractJavaScriptSymbols(
          cursor,
          code,
          filePath,
          graph,
        ...
        break;
      case "python":
        await this.extractPythonSymbols(
          cursor,
          code,
          filePath,
          graph,
  ...
  private async extractJavaScriptSymbols(
    ...
    const traverse = (node: Parser.SyntaxNode, scope: Scope) => {
      ...
      if (nodeType === "method_definition") {
        const nameNode = node.childForFieldName("name");
        if (nameNode) {
          const methodName = code.substring(
            nameNode.startIndex,
            nameNode.endIndex,
          );
          const symbol = this.createFunctionSymbol(
      ...
      if (nodeType === "class_declaration" || nodeType === "class") {
        const nameNode = node.childForFieldName("name");
        if (nameNode) {
          const className = code.substring(
            nameNode.startIndex,
            nameNode.endIndex,
          );
    ...
    const traverse = (node: Parser.SyntaxNode, scope: Scope) => {
      ...
      if (nodeType === "function_definition") {
        ...
        if (nameNode) {
          ...
          const symbol = this.createPythonFunctionSymbol(
            funcName,
            location,
            node,
            code,
      ...
      for (const param of paramsNode.children) {
        if (param.type === "identifier" || param.type === "typed_parameter") {
          const nameNode =
            ...
            : param;
          if (nameNode) {
            const paramName = code.substring(
              nameNode.startIndex,
              nameNode.endIndex,
            );
  ...
  private buildSignature(
    ...
    if (returnTypeNode) {
      const returnType = code.substring(
        returnTypeNode.startIndex,
        returnTypeNode.endIndex,
      );
  ...
  private extractCallExpression(
    ...
    if (functionNode) {
      const calleeName = code.substring(
  ...
  private addSymbol(
    ...
    const qualifiedName =
      scope.type === "global" ? symbol.name : `${scope.name}.${symbol.name}`;
