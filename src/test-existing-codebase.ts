...
async function testExistingCodebase() {
  ...
  for (const symbolName of testQueries) {
    ...
    if (symbol) {
      console.log(`  ✅ Found '${symbolName}' in ${queryTime}ms (${symbol.type})`);
    } else {
  ...
  console.log('\n\n📁 Per-File Analysis:');
  console.log('─'.repeat(70));
  console.log('File'.padEnd(40) + 'Symbols'.padEnd(10) + 'Unresolved'.padEnd(15));
  console.log('─'.repeat(70));

  const fileStats = [];
  for (const file of files.slice(0, 10)) { // Show top 10
  ...
  console.log(`  ${parsePerFile < 2000 ? '✅' : '❌'} Parse time < 2s per file: ${parsePerFile.toFixed(1)}ms`);
  console.log(`  ${avgQueryTime < 1 ? '✅' : '❌'} Query time < 1ms: ${avgQueryTime.toFixed(2)}ms`);
  console.log(`  ${warmTime < coldTime / 10 ? '✅' : '❌'} Incremental speedup > 10x: ${(coldTime / warmTime).toFixed(1)}x`);
  ...
  if (deadCode.length > 0) {
    ...
    for (const symbol of deadCode.slice(0, 5)) {
      console.log(`  • ${symbol.type} '${symbol.name}' at ${symbol.location.file}:${symbol.location.line}`);
    }
  ...
  for (const [caller, callees] of graph.callGraph.entries()) {
    ...
    if (callees.length > 0) {
      console.log(`  • '${caller}' calls: ${callees.slice(0, 3).join(', ')}${callees.length > 3 ? '...' : ''}`);
      callSamples++;
    }
  }
