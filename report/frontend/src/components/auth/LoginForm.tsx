...
export default function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.success) {
        // Handle specific error for unconfirmed email if your backend returns it
        if (response.error?.toLowerCase().includes('not verified') || 
            response.error?.toLowerCase().includes('otp')) {
      ...
      toast.success('Signed in successfully', {
        description: 'Welcome back to DevHQ!',
      });
    ...
    <div className="w-full max-w-md bg-white p-8 border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div className="space-y-4 mb-8">
        <button
          ...
          disabled={true}
          className="w-full flex items-center justify-center gap-3 oauth-button p-3 font-bold border-2 border-black rounded-full disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      <form onSubmit={handleLogin} className="space-y-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-2 px-4">
            <label className="block text-sm font-bold uppercase" htmlFor="password">
              Password
            </label>
            <Link
      ...
      <div className="mt-8 text-center">
        <p className="text-black font-medium">
          ...
          <Link href="/waitlist" className="font-bold underline decoration-2 decoration-[#ccff00] hover:bg-[#ccff00] transition-colors px-1">
            Join the waitlist
          </Link>
        </p>
      </div>
